...
| [`galerie_donnees_mixtes/`](./galerie_donnees_mixtes/) | **Étagère asynchrone optionnelle post-M4** — les gestes jamais pratiqués ensemble : **pipeline mixte tabulaire+texte** (ColumnTransformer + TF-IDF), **multi-classe déséquilibrée** (CV stratifiée, matrice 3×3), **coûts d'erreur asymétriques** (poids, seuils), scénarios de données. Compagnon direct des phases 3-4 de la feuille de route cas d'usage. Pas de livrable. |
| [`galerie_explicabilite/`](./galerie_explicabilite/) | **Étagère asynchrone optionnelle dès M6** — expliquer une décision de modèle : importance globale honnête (permutation vs impureté), **SHAP local** traduit en phrase pour le client, détection d'une **variable proxy** (passerelle C2), LIME en contraste. 1 notebook résolu + 1 notebook à faire sur ton propre modèle. Sert directement l'étape 5 de la feuille de route cas d'usage et la ligne « explicabilité » de la grille C4. Pas de livrable. |
| [`galerie_genai/`](./galerie_genai/) | **Étagère asynchrone optionnelle — à faire APRÈS ton cas d’usage certif** — RAG de bout en bout et son **évaluation** (hit@k, citations, questions pièges, seuil d’abstention), menu de dépannage RAG mesuré (normalisation, HyDE, reranking, chunking sémantique, fusion 🟠, juge d’ancrage 🟠), **agent from scratch** (boucle ReAct, outils typés, 5 garde-fous, prompt injection démontrée puis bornée) et **LoRA sur Colab**. Tout en local via Ollama, `MOCK_MODE` si Ollama est impossible. |
| [`cahier_vacances/`](./cahier_vacances/) | **Cahier de vacances été 2026** — menu de consolidation à la carte pour l'intersaison M4→M5 (🟢 entretien / 🎯 consolidation via les galeries / ⭐ mini cas d'usage), avec fiche de suivi et journal de l'été à déposer avant le 31 août. Non noté. |
...
| [`fiche_frameworks_deep_learning.md`](./fiche_frameworks_deep_learning.md) | **Carte des frameworks DL** — culture pure : situer scikit-learn / PyTorch / Keras-TensorFlow / Hugging Face (qui fait quoi, quel étage, quoi répondre en entretien), seconds rôles (ONNX, JAX), table symptôme → framework pour lire du code existant. Dès M4-B2, utile M5-M8 et entretiens. |
| [`notebook_lire_code_deep.ipynb`](./notebook_lire_code_deep.ipynb) | **Notebook de LECTURE compagnon de la carte des frameworks** — le même classifieur écrit en Keras puis PyTorch, plus la « troisième voie » (modèle pré-entraîné en 3 lignes). Sorties déjà exécutées : rien à installer. Questions d'observation + quiz « reconnaître du code hérité ». Après M4-B2. |
| [`feuille_route_cas_usage.md`](./feuille_route_cas_usage.md) | **Plan de vol du cas d'usage certif** — les 8 phases, quelle fiche ouvrir à quel moment, ce qui est faisable dès la fin de M4 vs ce qui attend M5-M8. À lire en premier au lancement du cas d'usage, puis à chaque changement de phase. |
...
| [`Ex-Cas-dusage/`](./Ex-Cas-dusage/) | **Exemple de livraison complète** — un cas d'usage rendu par un candidat d'une promo précédente : notebook final, journal de bord et présentation PDF. À ouvrir pour voir concrètement le niveau et le format attendus, **pas** un modèle à recopier (sujet et choix techniques différents des vôtres). |
